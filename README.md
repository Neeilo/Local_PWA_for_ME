...
`查` 讀近 30 天資料組成上下文後呼叫 Gemini API，用自然語言回答。記帳部分送的是**分類彙總而非逐筆**——逐筆一旦被筆數上限截斷，AI 會拿到半個月的資料卻不知情，然後自信地給出錯誤的總額；答錯比答不出來更糟。未完成任務則不受 30 天窗口限制，因為「未完成」是持續狀態而非時點事件。Gemini key 存 Apps Script 指令碼屬性（`GEMINI_API_KEY`），**不進 GitHub**——它在伺服器端使用，沒有前端密鑰那種「一定會被看到」的限制。
