...
5. 📎 **雜記** — 靈感與雜項，**類別選填**（文章／作品／系統／分享／其他）
...
雜記的類別存成內容前綴（`[作品] 做了一個 PWA`），寫進 `notes` 既有的 `text` 欄位，**分頁與欄位一個字都不用改**。代價是類別從「資料」降級成「約定」，要統計就得靠字串解析——對這個規模的系統划算。原本的成長頁（週回顧＋輸出追蹤）已移除，其中「輸出追蹤」以此形式併入雜記；兩者的舊資料從未上過雲端。ADR-009（2026-09-16）起前端不再讀回 `localStorage` 裡的舊 state，第一次雲端同步成功時就被只含五個模組的 state 整包覆寫——**實際上已經取不回來**（2026-09-24 查證）。成長頁要不要以別的形式回來，排在 grill-me。
...
- **ADR-009**（依 [ADR-009]，2026-09-16）：✅ 已完成
  ...
  - ✅ Phase 2-f（兩段式封存）：「封存已刪除的資料」按鈕接上 `?only=tombstones` 與 `archivePurge`。流程是**標記 → 匯出 → 確認 → 刪除**，一步都不能提前。10 項測試，SW v16→v17
  - ⚠️ **ADR 沒料到的現實：瀏覽器沒有「下載成功」這個事件**。`a.click()` 之後無從得知使用者按了儲存、取消、還是根本沒看到對話框。所以那個「下載成功訊號」只能由人給——下載後跳確認框，按下去才刪。人就是那個訊號，這是誠實的做法，不是偷懶
  - 📌 **鍵一律由後端算**（`?only=tombstones` 連 `keys` 一起回傳）。前端自己算的話，`doGet` 走 `JSON.stringify` 會把 Date 轉成 UTC ISO 字串，在 UTC+8 切出來的日期可能差一天；鍵一對不上，`archivePurge` 會**安靜地一筆都刪不掉**——沒有錯誤訊息，只是沒有效果。同一套規則只留一份實作，就沒有對不上的可能（與 DATA-05 同一個根）
  - ✅ Phase 2-g（週期提醒）：任務表單加到期日與週期（選填、預設摺疊）；打勾時走後端新的 `completeRecurring`，由後端生出下一期。19 項測試，SW v17→v18
  - ⚠️ **ADR 沒寫到、不處理就會出事**：重複打勾會不會生出兩列。使用者把已完成的週期任務取消打勾再重新打勾，天真的實作會再生一期。解法不需要新欄位——**生出下一期的同時把週期設定搬到新的那一列、從原列清掉**。原列從此不是週期任務，再怎麼打勾都不會再生。週期本來就該跟著「還沒做的那一期」走
  - 📌 下一期的日期算法**只留在後端一份**，前端打勾時把判斷交給後端而不是自己重算。兩份實作遲早漂移，而漂移的那天沒有人會收到通知——它只會安靜地算錯日期
  - 📌 只填週期沒填到期日會被擋下並說明原因，不安靜忽略；後端接手失敗時前端退回普通 upsert，至少「這一期完成了」這個事實不會掉
  - ✅ Phase 2-h（白板與到期區塊）：`tasks`／`expenses`／`notes` 列上的 📌 釘選，儀表板一張卡片用 `scroll-snap-type: x mandatory` 橫向滑動切換「共用白板」與「快到期了」。13 項測試，SW v18→v19
  - 📌 **白板刻意不套「只看我的」**：這是共用白板，看不到別人釘的東西就失去意義了
  - 📌 **已逾期獨立一塊、排最前面**（ADR 未規定）。把過期項目塞進「1 天內」會是假話；最急的東西放最下面也沒有道理
  - ⚠️ **到期門檻在前後端各有一份實作**（Apps Script 與瀏覽器沒辦法共用），這正是本輪一直在避免的事。改用測試把兩邊釘在一起：`tests/adr-009-board-due.test.mjs` 會**同時載入兩份原始碼**比對門檻數字與分類結果，有人改了一邊沒改另一邊就會紅燈
  - **ADR-009 至此全部完成。** 共 8 個 PR（#17／#19～#24），148 項測試，每一輪都附突變驗證

### ADR-009 完成後的回顧

**根治的**：`replaceAll` 整包覆蓋退場（DATA-01 不是被防住，是不存在了）、刪除變成送得出去的事、讀取失敗不再偽裝成空表、推送失敗不再無感知。

**本輪發現、ADR 沒寫到、但不處理就會出事的六件事**（都已處理並附測試）：

1. `cloudGet` 把讀取失敗吞成空陣列 → 覆蓋式 pull 之下**一次網路抖動清光五張表**
2. Sheet 的 Date 物件與前端的 `YYYY-MM-DD` 字串對不上 → 複合鍵永遠失效、每次 upsert 退化成 append
3. 即時單筆 upsert 沒有重試 → **比改版前更糟**（舊的整包覆蓋天生有重試），因此加了待送佇列
4. 瀏覽器沒有「下載成功」事件 → 封存的第二段訊號改由人給
5. 週期任務重複打勾會生出兩列 → 生下一期時把週期設定搬到新列，原列從此不是週期任務
6. 推播失敗若標記 `notified` → 這筆從此不再提醒而使用者不知道

**仍待裁決（不影響運作）**：月底錨點漂移要不要加欄位；離線是否保留唯讀本機快取的逃生門；到期門檻第 6 天的歸屬（ADR 寫「7 天以上」，現採 ≤5／其餘）。
