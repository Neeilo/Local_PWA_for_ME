...
| 類別 | 形式 | 為什麼 |
|---|---|---|
| `expenses` | **分類彙總**，且**本月與近 30 天各一組** | 見下方 |
...
**「這個月」不等於「近 30 天」。** 這條踩過一次：只給近 30 天的合計時，使用者問「這個月花多少」，AI 就把近 30 天的數字報出來——那裡面含著上個月下旬的支出，而且跟 App 首頁的「本月支出」對不起來。使用者會拿兩邊對照，然後發現數字不合。

所以彙總同時給兩組，並在規則裡明文禁止混用或相加：

```
▍本月（2026-09-01 ~ 2026-09-10）
▍近 30 天（2026-08-11 ~ 2026-09-10）
```

問法沒指明期間時預設用本月，並要求 AI 在回答中說明。本月的起算日是月曆月 1 號，與前端 `monthKey()` 的定義一字不差——**兩邊對得起來，才是這組數字有用的前提**。

逐筆的區塊（任務、複盤、心情）標題都明寫「近 30 天」，免得被誤當本月。
...
`GEMINI_MAX_OUTPUT_TOKENS = 2048`，給得比實際需要寬很多，這是刻意的。

Gemini 2.5／3 系列的 **thinking tokens 也計入 `maxOutputTokens`**，而思考往往吃掉數百個。上限抓太緊時，模型會在還沒寫完答案就撞到天花板，使用者收到半截話。我們只要 5 行回覆，2048 綽綽有餘，多出來的額度是留給思考的。

更重要的是**截斷不再是靜默的**：程式會檢查 `finishReason`。

| 情況 | 行為 |
|---|---|
| 有文字 + `MAX_TOKENS` | 照樣送出，但附上「⚠️ 回應太長被截斷了」，`logs` 標記為「已回覆（被截斷）」 |
| 沒文字 + `MAX_TOKENS` | 走失敗路徑，原因記為「回應被 token 上限截斷且無內容」（與單純的「回應沒有內容」區分） |

半截答案仍比沒答案有用，但**悄悄把截斷的內容當成完整答案送出去，是這裡最不該犯的錯**。
